...
// La logique pour envoyer l'email à Resend
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
  // Au lieu de alert(), on redirige vers ta nouvelle page
  ...
  alert("Oups, un petit souci technique."); 
      ...
      console.error(err);
      alert("La Forge est injoignable. Vérifie ta connexion.");
    }
  };
  return (
      ...
      <div className='mx-auto flex h-full max-w-7xl flex-col gap-16 px-4 sm:px-6 lg:px-8'>
        ...
        <div className='flex max-w-4xl flex-col items-center gap-4 self-center text-center'>
          ...
          <h1 className='text-3xl leading-[1.29167] font-semibold text-balance sm:text-4xl lg:text-5xl'>
            La Forge Nexus DevOps
          </h1>
          <p className='text-muted-foreground mx-auto max-w-2xl text-xl'>
            Bienvenue sur mon blog dédié à la culture DevOps.Je documente ici mes projets, mes labs afin de t'aider à progresser.
          </p>
          {/* AJOUTE TON PINGOUIN ICI */}
<div className='py-4'>
  <img 
    src="/images/nexusblog.png" 
    alt="Nexus Penguin" 
  ...
  onSubmit={handleSubmit} // 1. On branche la fonction ici
>
  <Input 
    name='email' 
    type='email' 
  ...
  <Button 
    ...
    type='submit' // 2. On retire 'asChild' et le '<a>'
  >
    S'abonner
  </Button>
</form>
